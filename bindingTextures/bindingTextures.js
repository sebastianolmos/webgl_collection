...
function main() {
    ...
    // Connect slider and text
    let mixSlider = document.getElementById("mixRange");
    let mixOutput = document.getElementById("textValue");
    mixOutput.innerHTML = (mixSlider.value / 100).toString(); // Display the default slider value
    // Update the current slider value (each time you drag the slider handle)
    mixSlider.oninput = function() {
        mixValue = this.value / 100;
        mixOutput.innerHTML = mixValue.toString();
    }
    ...
    const bindTexShader = new Simple2TextureShader(gl);
    ...
    quadShape.createTexture(gl, "../assets/dragon.png", gl.RGBA, gl.CLAMP_TO_EDGE, gl.NEAREST);
    ...
    lineShape.createTexture(gl, "../assets/dragon.png", gl.RGBA, gl.CLAMP_TO_EDGE, gl.NEAREST);
    ...
    function drawScene() {
        ...
        gl.useProgram(bindTexShader.shaderProgram);
        gl.uniform1f(bindTexShader.uniformLocations.mixValue, mixValue);

        if (triangleMode){
            // Draw the Shape with Triangle mode
            bindTexShader.drawCall(quadShape, gl.TRIANGLES);
        }
        else {
            // Draw the Shape with Line mode
            bindTexShader.drawCall(lineShape, gl.LINES);
        }
    }
